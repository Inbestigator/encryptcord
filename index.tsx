...
const ChatBarIcon: ChatBarButtonFactory = ({ isMainChat }) => {
  ...
  return (
    <ChatBarButton
      tooltip={
        enabled ? "Send Unencrypted Messages" : "Send Encrypted Messages"
      }
      onClick={async () => {
        if (
...
export default definePlugin({
  ...
  async interactionHandler(interaction) {
    const sender = await UserUtils.getUser(interaction.application_id).catch(
      () => null
    );
    if (
      !sender ||
  ...
  flux: {
    async MESSAGE_CREATE({
      ...
      if (
        ...
        switch (message.content.toLowerCase().split("```")[0]) {
          case "groupdata":
            ...
            break;
          case "join":
            if (encryptcordGroupMembers[UserStore.getCurrentUser().id].child)
              return;
      ...
      const dmChannelId = await PrivateChannelsStore.getOrEnsurePrivateChannel(
        message.author.id
      );
...
async function sendTempMessage(
  ...
  const channelId = dm
    ? await PrivateChannelsStore.getOrEnsurePrivateChannel(recipientId)
    : recipientId;
...
async function handleGroupData(groupData) {
  ...
  await MessageActions.receiveMessage(
    groupData.channel,
    await createMessage("", UserStore.getCurrentUser().id, groupData.channel, 7)
  );
  setEnabled(true);
}
